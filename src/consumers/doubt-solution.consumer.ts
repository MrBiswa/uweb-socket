...
export class DoubtSolutionConsumer implements OnModuleInit {
    ...
    constructor(io: UWebSockets) {
        this.kafkaTopic = kafkaConfig.consumer.topics.DOUBT_SOLUTION_TOPIC;
        this.kafkaGrp = kafkaConfig.consumer.groups.DOUBT_SOLUTION_GROUP;
    ...
    onModuleInit() {
        if (this.kafkaTopic && this.kafkaGrp) {
            ...
            logger.log("Wrong topic and group in DoubtConsumer: ", this.kafkaTopic, this.kafkaGrp);
        }
    }
    ...
    async messageProcessor(event: MessagePayload) {
        try {
            ...
            logger.log("Emitting doubt kafka data: ", kafkaData);
            if (kafkaData) {
                const parsedData = JSON.parse(kafkaData);
                const { roomId } = parsedData;
                this.io.in(roomId, EmitContext.DOUBT_SOLUTION, parsedData);
            }
        } catch (err) {
            logger.error("Error while processing DoubtSolutionConsumer message: ", err);
        }
    }
}
