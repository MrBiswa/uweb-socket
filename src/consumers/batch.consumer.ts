...
export class BatchConsumer implements OnModuleInit {
    ...
    constructor(io: UWebSockets) {
        this.kafkaTopic = kafkaConfig.consumer.topics.BATCH_TOPIC;
        this.kafkaGrp = kafkaConfig.consumer.groups.BATCH_GROUP;
    ...
    onModuleInit() {
        if (this.kafkaTopic && this.kafkaGrp) {
            ...
            logger.log("Wrong topic and group in BatchConsumer: ", this.kafkaTopic, this.kafkaGrp);
        }
    }
    ...
    async messageProcessor(event: MessagePayload) {
        try {
            ...
            logger.log("Emitting batch kafka data: ", kafkaData);
            if (kafkaData) {
                const parsedData = JSON.parse(kafkaData);
                const { room } = parsedData;
                // delete parsedData?.room;
                this.io.in(room, EmitContext.BATCH, parsedData);
            }
        } catch (err) {
            logger.error("Error while processing BatchConsumer message: ", err);
        }
    }
}
