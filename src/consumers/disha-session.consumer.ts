...
export class DishaSessionConsumer implements OnModuleInit {
    ...
    constructor(io: UWebSockets) {
        this.kafkaTopic = kafkaConfig.consumer.topics.DISHA_SESSION_TOPIC;
        this.kafkaGrp = kafkaConfig.consumer.groups.DISHA_SESSION_GROUP;
            ...
            const runConfig = {
                eachMessage: async (event: MessagePayload) => {
                    try {
                        ...
                        logger.warn(`DISHA SESSION KAFKA ERROR:: topic: ${this.kafkaTopic}, event: ${JSON.stringify(event)}, error: ${error}`);
                    }
                },
            };
    ...
    async messageProcessor(event: MessagePayload) {
        try {
            ...
            if (kafkaData) {
                ...
                if (room) {
                    this.io.in(room, EmitContext.DISHA_SESSION, parsedData);
                }
            }
        } catch (err) {
            logger.error("Error while processing DishaSessionConsumer message: ", err);
        }
    }
}
