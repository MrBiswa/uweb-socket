...
export class AIGuruChatConsumer implements OnModuleInit {
    ...
    constructor(io: UWebSockets) {
        this.kafkaTopic = kafkaConfig.consumer.topics.AI_GURU_SUGGESTIVE_QUESTION_TOPIC;
        this.kafkaGrp = kafkaConfig.consumer.groups.AI_GURU_CHAT_GROUP;
    ...
    onModuleInit() {
        if (this.kafkaTopic && this.kafkaGrp) {
            ...
            logger.log("Wrong topic and group in AIChatGuruConsumer: ", this.kafkaTopic, this.kafkaGrp);
        }
    }
            ...
            const runConfig = {
                eachMessage: async (event: MessagePayload) => {
                    try {
                        ...
                        logger.warn(
                            `AI GURU CHAT KAFKA ERROR:: topic: ${this.kafkaTopic}, event: ${JSON.stringify(event)}, error: ${error}`,
                        );
                    }
                },
            };
    ...
    async messageProcessor(event: MessagePayload) {
        try {
            ...
            if (kafkaData) {
                ...
                this.io.in(room, EmitContext.AI_GURU_CHAT, parsedData);
            }
        } catch (err) {
