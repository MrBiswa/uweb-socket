...
export class EmojiConsumer implements OnModuleInit {
    ...
    constructor(io: UWebSockets) {
        this.kafkaTopic = kafkaConfig.consumer.topics.EMOJI_TOPIC;
        this.kafkaGrp = kafkaConfig.consumer.groups.EMOJI_GROUP;
    ...
    onModuleInit() {
        if (this.kafkaTopic && this.kafkaGrp) {
            ...
            logger.log("Wrong topic and group in EmojiConsumer: ", this.kafkaTopic, this.kafkaGrp);
        }
    }
        ...
        try {
            ...
            const runConfig = {
                eachMessage: async (event: MessagePayload) => {
                    ...
                    // } catch (error) {
                    //     logger.warn(`Emoji KAFKA ERROR:: topic: ${this.kafkaTopic}, event: ${JSON.stringify(event)}, error: ${error}`);
                    // }
                },
            };
    ...
    async messageProcessor(event: MessagePayload) {
        try {
            ...
            logger.log("Emitting Emoji kafka data: ", kafkaData);
            if (kafkaData) {
                const parsedData = JSON.parse(kafkaData);
                const { room } = parsedData;
                // delete parsedData?.room;
                this.io.in(room, EmitContext.EMOJI, parsedData);
            }
        } catch (err) {
            logger.error("Error while processing EmojiConsumer message: ", err);
        }
    }
}
